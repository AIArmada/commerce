...
| OS5 | `commerce-support` | `OwnerContext` static `$fallback` can leak in Octane. | Partially true | `packages/commerce-support/src/Support/OwnerContext.php:33,115-141,174-202`; fallback is static but `withOwner()` restores in `finally`; HTTP context stores state on request attributes. Regression coverage now exercises exception restoration, non-HTTP fallback restoration, and per-request isolation. | Done; no production storage change needed without a reproduced leak. | Passed `tests/src/CommerceSupport/OwnerContextTest.php` and PHPStan level 6 on `packages/commerce-support/src`. |
...
| I2 | `checkout`, `chip`, `docs` | No package-local runner configs. | Partially true | No package-local `phpunit.xml`/`pest.xml`/`phpstan.neon` found; root `phpunit.xml` and `tests/src` exist and are current monorepo pattern. | Decision: root runner only. Use `./vendor/bin/pest --parallel tests/src/<Package>` for package-scoped runs. No per-package config files. | None. |
...
| DEP2 | `cashier` | `laravel/cashier` is hard-used but only suggested. | True | `packages/cashier/composer.json:20-27`; multiple `use Laravel\Cashier\...` in `packages/cashier/src/Gateways/Stripe/*` and `StripeGateway.php`. | Decision: isolate Stripe at the runtime layer; `GatewayManager` guards resolution with `class_exists(Cashier::class)`, filters `stripe` from `supportedGateways()` and `supportsGateway()` when absent, and `createStripeDriver()` throws a clear `GatewayNotFoundException` with install hint. Stripe files stay in cashier but their `use Laravel\Cashier\*` references only resolve when the dependency is present. | `tests/src/Cashier/Unit/GatewayManagerStripeIsolationTest.php`, full cashier suite (245 passed), PHPStan level 6 clean. |
...
| CQ4 | `products` | Ten models use `$guarded = ['id']`. | True | Completed in current tree: all 10 product models now use explicit `$fillable`; `tests/src/Products/MassAssignmentGuardTest.php` covers guarded `id`. | Done. | Passed `tests/src/Products/MassAssignmentGuardTest.php` and PHPStan level 6 on `packages/products/src`. |
| CQ5 | `customers` | Five models use `$guarded = ['id']`. | True | Completed in current tree: all 5 customer models now use explicit `$fillable`; `tests/src/Customers/MassAssignmentGuardTest.php` covers guarded `id`. | Done. | Passed `tests/src/Customers/MassAssignmentGuardTest.php` and PHPStan level 6 on `packages/customers/src`. |
...
| CQ12 | `products` | Policy helper methods are duplicated. | True | Completed in current tree: shared `HandlesOwnerScoping` trait exists under `packages/products/src/Policies/Concerns`; six product policies use it. | Done. | Passed `tests/src/Products/PolicyOwnerScopingTest.php` and PHPStan level 6 on `packages/products/src`. |
...
| CQ24 | `checkout` | `checkout_sessions` has 11 JSON columns and standard B-tree indexes cannot query JSON paths. | Partially true | Migration has configurable JSON columns at `2024_01_01_000001_create_checkout_sessions_table.php:35-47`; count is 8 JSON columns, not 11; config has `database.json_column_type`. Index concern is a design/performance risk, not proven bug. | No-fix: zero SQL-level JSON path queries exist in `src/`. All JSON access is through Eloquent `array` casts. No indexes to add. | None. |
...
| CQ26 | `membership` | Contracts lack no-op defaults and actions use `app()->bound()`. | True | `ApproveMembershipApplicationAction`, `RejectMembershipApplicationAction`, `ApplyForMembershipAction`, `RemoveMemberAction`, `ChangeMemberRoleAction` call `app()->bound()` for contracts. | Decision: keep `app()->bound()` pattern. Canonical Laravel opt-in contract pattern. Adding 5 default no-op classes is boilerplate with no behavior change. | None. |
| CQ27 | `moderation` | No contracts/interfaces; actions are concrete singletons. | True | `ModerationServiceProvider.php:25-26` registers concrete `BlockEntityAction` and `RecordModerationAction`; no contracts found. | Fixed: added `Contracts\BlocksEntity` and `Contracts\RecordsModerationAction` interfaces. Actions implement them. Provider binds contracts to concrete. Traits use contracts. 3 binding tests added. | 3 binding tests in `InstallationTest.php`. |
| CQ28 | `signals` | No domain events; 21 listeners consume external events. | Partially true | `packages/signals/src/Listeners` contains 21 listeners; package emits jobs/alerts but domain-event model is intentionally integration-heavy per docs. | Decision (in-monorepo only): keep listener-based pull model; dead mapper code removed. | Done. |
...
| A1 | `addressing` | `area_sources` config is only consumed by command, no provider auto-registration. | True | `config/addressing.php:23-24`; `ImportAddressAreasCommand.php:23-51`; docs say sources are available to command. | Fixed: added `bootingPackage()` in `AddressingServiceProvider.php` that iterates `config('addressing.area_sources')` and registers each as a container singleton. Apps drop their source class into config — provider auto-picks it up. | Provider tests (no test infrastructure exists in this package yet). |
| A2 | `checkout` | Eight hard dependencies risk cascading breakage. | True | Same evidence as CQ25. | No change: all 8 deps (`cart`, `commerce-support`, `customers`, `docs`, `orders`, `pricing`, `products`, `shipping`) are actively used in checkout orchestration. Softening would break the workflow. No cascade evidence found. | None. |
...
| A4 | `commerce-support` | `OwnerContext` static fallback may leak in Octane. | Partially true | Same evidence as OS5; added regression coverage for exception restoration, fallback cleanup, and request-instance isolation. | Done; no leak reproduced. | Same as OS5. |
| A5 | `events` | Many event models rely on provider scope registration. | Partially true | Same evidence as OS4. | Verified: `CrossTenantIsolationTest.php` covers 12+ event models with owner isolation, polymorphic scopes, and write guards. Scopes properly registered in provider. No change needed. | `CrossTenantIsolationTest.php`. |
| A6 | `engagement` | `BookmarkCollection` lacks cascades for collection items. | True | Completed in current tree: `BookmarkCollection::booted()` deletes related collection items; `tests/src/Engagement/BookmarkCollectionCascadeTest.php` covers deletion behavior. | Done. | Passed `tests/src/Engagement/BookmarkCollectionCascadeTest.php` and PHPStan level 6 on `packages/engagement/src`. |
| A7 | `filament-shipping` | Owner scoping code is repetitive. | Partially true | Multiple resources/pages repeat `getNavigationGroup()` and owner scoping patterns; no defect proven. | No-fix: navigation pattern already follows repo config-backed standard. Owner scoping in `getEloquentQuery()` is intentionally per-resource. Abstract base would be over-engineering. | None. |
...
| CS1 | multiple | Three different `$guarded`/`$fillable` approaches exist. | True | Payment models were standardized earlier; products/customers were standardized in current tree with explicit `$fillable`. | Done for verified rows in this ledger. | Payment, products, and customers mass-assignment tests. |
| CS2 | filament packages | Four owner-scoping patterns in Filament. | Partially true | Verified examples: direct `forOwner`, `OwnerUiScope`, parent query relying on global scopes, `withoutGlobalScopes`. | Decision: pick by use case. `forOwner()` for read-write resources, `OwnerUiScope` for widgets/pages with multiple read paths, `withoutGlobalScopes()` only for intentionally global resources (permissions/roles). Standard documented for future filament work. | Cross-tenant resource/widget tests. |
...
| CS7 | `checkout`, `chip`, `docs` | No package-local test config. | Partially true | Same as I2. | Decision (same as I2): root runner only. | None. |
...
| R19 | `filament-cashier-chip` | Add owner scoping to dashboard widgets. | True | Verified current tree: `cashier-chip` `Subscription` is tenant-owned via `HasOwner`; dashboard widgets used bare subscription queries. Fixed with shared owner-safe `subscriptionQuery()` and fail-closed Cashier CHIP owner scopes/resources. | Done. | `tests/src/CashierChip/Feature/CrossTenantIsolationTest.php`, `tests/src/FilamentCashierChip/Feature/CrossTenantIsolationTest.php`, and `tests/src/FilamentCashierChip/Feature/WidgetsCoverageTest.php`. |
...
| R29 | `products` | Remove duplicate policy helpers. | Derived from true finding | Backed by CQ12; completed via shared `HandlesOwnerScoping` trait. | Done. | `tests/src/Products/PolicyOwnerScopingTest.php` added by user. |
...
| R34 | `products`, `customers` | Standardize `$fillable`. | Derived from true finding | Backed by CQ4/CQ5; completed for all 15 models. | Done. | `tests/src/Products/MassAssignmentGuardTest.php` and `tests/src/Customers/MassAssignmentGuardTest.php` added by user. |
...
- Product and customer model mass assignment was standardized to explicit `$fillable`, with guarded `id` regression tests.
- Product policy owner-scoping helpers were extracted to a shared trait, with policy regression tests.
- Engagement bookmark collection deletes now cascade to collection items in application logic, with deletion regression tests.
...
- Filament Cashier CHIP dashboard widgets now query subscriptions through an owner-safe helper; Cashier CHIP explicit `forOwner()` scopes and base resources fail closed when owner mode is enabled without a resolved owner or explicit global context.
- Commerce Support owner context leak concerns are covered by exception, non-HTTP fallback, and HTTP request-isolation regression tests; no leak reproduced.
...
- Signals dead mapper code (`MapCommerceEventToSignalInterface`, `RecordSignalFromEvent`, 5 mappers, `signals.event_mappers` tag) was removed; the 21 per-event listeners wired by `CommerceSignalsIntegrationRegistrar` are the single integration path.
- Cashier Stripe isolation: `GatewayManager::supportedGateways()`, `supportsGateway()`, and `createStripeDriver()` guards added behind `class_exists(\Laravel\Cashier\Cashier::class)`. No hard dependency introduced. (`DEP2`)
- Moderation contracts: `Contracts\BlocksEntity` and `Contracts\RecordsModerationAction` extracted. Actions implement them. Provider binds contracts. Traits use contracts. 57/57 tests pass. (`CQ27`)
- Addressing `area_sources`: `AddressingServiceProvider::bootingPackage()` now iterates config and registers each as a container singleton. Apps add their source class to config — provider auto-picks it up. (`A1`)
- Exception hierarchies: `CashierChipException` base for cashier-chip (7 exceptions), `InventoryException` base for inventory (1 exception). All 14 other packages have zero custom exceptions. (`R23`)
...
- Continuation pass also passed for `inventory`, `jnt`, `cashier`, `filament-cashier`, `filament-addressing`, `commerce-support`, and `checkout`.
- Takeover pass also passed for `cashier-chip` and `filament-cashier-chip` after the Cashier CHIP owner-scope/widget fixes.
- Takeover audit also passed for `products`, `customers`, and `engagement` after the user-completed mass-assignment, policy, and bookmark cascade fixes.
- Takeover audit also passed for `commerce-support` after the OwnerContext leak regression coverage.
...
- Continuation pass also passed: `tests/src/Inventory/Unit/InventoryServiceTest.php`, `tests/src/Inventory/Unit/OwnerScopingTest.php`, `tests/src/Inventory/Unit/Actions/ShipInventoryTest.php`, `tests/src/Inventory/Unit/Actions/TransferInventoryTest.php`, `tests/src/Jnt/Feature/Commands/ConfigCheckCommandTest.php`, `tests/src/Jnt/Feature/Commands/HealthCheckCommandTest.php`, `tests/src/Jnt/Feature/Commands/OrderCreateCommandTest.php`, `tests/src/Jnt/Unit/Console/ConsoleCoverageTest.php`, `tests/src/Cashier/Unit/BillableTest.php`, `tests/src/FilamentCashier/Feature/CoverageBoostTest.php`, and `tests/src/CommerceSupport/NeedsOwnerMiddlewareTest.php`.
- Takeover pass also passed: `tests/src/CashierChip/Feature/CrossTenantIsolationTest.php`, `tests/src/FilamentCashierChip/Feature/CrossTenantIsolationTest.php`, and `tests/src/FilamentCashierChip/Feature/WidgetsCoverageTest.php`.
- Takeover audit also passed: `tests/src/Products/MassAssignmentGuardTest.php`, `tests/src/Customers/MassAssignmentGuardTest.php`, `tests/src/Products/PolicyOwnerScopingTest.php`, and `tests/src/Engagement/BookmarkCollectionCascadeTest.php`.
- A4/OS5 coverage passed: `tests/src/CommerceSupport/OwnerContextTest.php`.
