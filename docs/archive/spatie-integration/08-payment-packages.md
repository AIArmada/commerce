...
> **Historical note:** The baseline analysis below describes the pre-unification state that motivated this blueprint. CHIP webhook storage, retry handling, and audit coverage are now implemented; use this document as an architectural pattern for remaining payment packages, and check `PROGRESS.md` for the archived rollout record.
