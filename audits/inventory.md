...
**Migration Required: NO** — migration track completed 2026-09-07, see `migration-record.md#inventory`
...
- Migration Required: YES only for the optional movement composite index (ships standalone).
...
- `down()` methods present everywhere although not required — harmless, keep. The migration-track files (dedicated `reservation_group_id` migration, decimal drops) carry no `down()` by design — re-runs are guarded; rollbacks are manual.
...
|---|---|---|---|
| `products` (`Product`/`Variant` implement `Inventoryable`, use `HasInventory`) | trait methods, availability helpers | A8 moves mutators off the trait | Migrate mutator call sites to `InventoryService`; re-run product stock tests |
