...
beforeEach(function (): void {
    ...
    config()->set('cart.snapshots.abandonment_detection_minutes', 30);
...
it('exits successfully when abandonment tracking feature is disabled', function (): void {
    config()->set('cart.snapshots.abandonment_tracking', false);

    $this->artisan('cart:clear-abandoned --mark-only --dry-run')
        ->expectsOutputToContain('Snapshot abandonment tracking is disabled via cart.snapshots.abandonment_tracking.')
        ->assertSuccessful();
});

it('skips malformed owner tuples during all-owner abandoned-cart marking', function (): void {
    $table = config('cart.database.tables.snapshots', 'cart_snapshots');
    $now = now();
    ...
    $this->artisan('cart:clear-abandoned --mark-only --all-owners --dry-run')
        ->expectsOutputToContain('Marked 1 snapshot(s) as abandoned across owners.')
        ->assertSuccessful();
});
...
it('aborts in strict mode when malformed owner tuples are encountered', function (): void {
    ...
    $this->artisan('cart:clear-abandoned --mark-only --all-owners --dry-run --strict-owner-tuples')
        ->assertFailed();
});
...
it('requires explicit confirmation flag for non-dry all-owner mutation', function (): void {
    $this->artisan('cart:clear-abandoned --mark-only --all-owners')
        ->expectsOutputToContain('Multi-owner snapshot mutation requires --confirm-all-owners')
        ->assertFailed();
});
...
it('aborts when affected carts exceed max-affected threshold', function (): void {
    ...
    $this->artisan('cart:clear-abandoned --mark-only --all-owners --confirm-all-owners --max-affected=1')
