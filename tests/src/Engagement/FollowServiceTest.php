...
beforeEach(function () {
    ...
    {
        public function getMorphClass(): string { return 'speaker'; }
        public function getKey(): string { return 'speaker-1'; }
    };
});

it('creates a follow', function () {
    $follow = $this->manager->follow($this->actor, $this->subject);

    expect($follow->status)->toBe(Follow::STATUS_ACTIVE);
});

it('prevents duplicate active follows', function () {
...
it('unfollows without deleting', function () {
    $this->manager->follow($this->actor, $this->subject);
