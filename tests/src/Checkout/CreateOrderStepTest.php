...
it('refreshes voucher-driven affiliate overrides before creating order metadata', function (): void {
    ...
    Cart::setMetadata('affiliate', [
        'affiliate_id' => 'affiliate-two-id',
        'affiliate_code' => 'AFFE2E2',
        'attribution_id' => 'attr-two',
    ]);
    ...
    expect(data_get($session->cart_snapshot, 'metadata.affiliate.affiliate_code'))->toBe('AFFE2E2');
    ...
    expect(data_get($session->cart_snapshot, 'metadata.affiliate.affiliate_code'))->toBe('AFFE2E3')
        ->and(data_get($session->cart_snapshot, 'metadata.affiliate.voucher_code'))->toBe('AFFE2E3SAVE5')
    ...
    expect($createOrderResult->isSuccessful())->toBeTrue()
        ->and(data_get($capturedOrderData, 'metadata.affiliate_code'))->toBe('AFFE2E3')
