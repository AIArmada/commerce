...
it('defines a single-level administrative hierarchy', function (): void {
    $hierarchies = app(SolomonIslandsGeographyProvider::class)->addressHierarchies();
...
it('imports the Solomon Islander tree with state links', function (): void {
    $this->seedCountry('SB');

    $result = app(SeedCountryGeographiesAction::class)->execute('SB');
    $country = AddressCountry::query()->where('iso2', 'SB')->firstOrFail();

    expect($result['seeded'])->toContain('SB')
