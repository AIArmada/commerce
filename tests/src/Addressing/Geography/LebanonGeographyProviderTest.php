...
it('seeds all 8 Lebanese states', function (): void {
    ...
    app(LebanonGeographyProvider::class)->seed($country);

    $state->refresh();

    expect($state->name)->toBe('Akkar')
        ->and(State::query()->where('country_id', $country->id)->count())->toBe(8);
});

it('maps every Lebanese state code to its area', function (): void {
    $mappings = app(LebanonGeographyProvider::class)->stateAreaMappings();

    expect($mappings)->toHaveCount(8)
        ->and(array_map('strval', array_keys($mappings)))->toBe(['AK', 'BH', 'BA', 'BI', 'JL', 'NA', 'AS', 'JA']);
});
