...
it('seeds all 6 Gambian states', function (): void {
    ...
    app(GambiaGeographyProvider::class)->seed($country);

    $state->refresh();

    expect($state->name)->toBe('Banjul')
        ->and(State::query()->where('country_id', $country->id)->count())->toBe(6);
});

it('maps every Gambian state code to its area', function (): void {
    $mappings = app(GambiaGeographyProvider::class)->stateAreaMappings();

    expect($mappings)->toHaveCount(6)
        ->and(array_map('strval', array_keys($mappings)))->toBe(['B', 'M', 'L', 'N', 'U', 'W']);
});
