...
it('seeds all 4 Bahraini states', function (): void {
    ...
    app(BahrainGeographyProvider::class)->seed($country);

    $state->refresh();

    expect($state->name)->toBe('Northern')
        ->and(State::query()->where('country_id', $country->id)->count())->toBe(4);
});

it('maps every Bahraini state code to its area', function (): void {
    $mappings = app(BahrainGeographyProvider::class)->stateAreaMappings();

    expect($mappings)->toHaveCount(4)
        ->and(array_map('strval', array_keys($mappings)))->toBe(['13', '14', '15', '17']);
});
