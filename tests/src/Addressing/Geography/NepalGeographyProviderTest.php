...
it('seeds all 7 Nepali states', function (): void {
    ...
    $state = State::query()->create([
        ...
        'code' => 'P3',
        'name' => 'Bagmati (legacy)',
        'label' => 'Bagmati (legacy)',
    ]);

    app(NepalGeographyProvider::class)->seed($country);

    $state->refresh();

    expect($state->name)->toBe('Bagmati')
        ->and(State::query()->where('country_id', $country->id)->count())->toBe(7);
});

it('maps every Nepali state code to its area', function (): void {
    $mappings = app(NepalGeographyProvider::class)->stateAreaMappings();

    expect($mappings)->toHaveCount(7)
        ->and(array_map('strval', array_keys($mappings)))->toBe(['P3', 'P4', 'P6', 'P1', 'P5', 'P2', 'P7']);
});
