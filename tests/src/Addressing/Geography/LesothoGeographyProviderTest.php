...
it('seeds all 10 Basotho states', function (): void {
    ...
    app(LesothoGeographyProvider::class)->seed($country);

    $state->refresh();

    expect($state->name)->toBe('Berea')
        ->and(State::query()->where('country_id', $country->id)->count())->toBe(10);
});

it('maps every Basotho state code to its area', function (): void {
    $mappings = app(LesothoGeographyProvider::class)->stateAreaMappings();

    expect($mappings)->toHaveCount(10)
        ->and(array_map('strval', array_keys($mappings)))->toBe(['D', 'B', 'C', 'E', 'A', 'F', 'J', 'H', 'G', 'K']);
});
