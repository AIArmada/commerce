...
it('defines a single-level administrative hierarchy', function (): void {
    $hierarchies = app(DominicaGeographyProvider::class)->addressHierarchies();
...
it('imports the Dominican tree with state links', function (): void {
    ...
    $result = app(SeedCountryGeographiesAction::class)->execute('DM');
    $country = AddressCountry::query()->where('iso2', 'DM')->firstOrFail();

    expect($result['seeded'])->toContain('DM')
        ...
        ->and(AddressArea::query()->where('country_id', $country->id)->where('type', 'parish')->count())->toBe(10)
