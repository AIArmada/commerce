...
it('can get purchase id', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData()));
...
it('can check if payment is succeeded', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'success'])));
...
it('can check if payment is failed', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'failed'])));

    expect($payment->isFailed())->toBeTrue();
});

it('can check if payment is expired', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'expired'])));

    expect($payment->isExpired())->toBeTrue();
});

it('can check if payment is cancelled', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'cancelled'])));

    expect($payment->isCancelled())->toBeTrue();
});

it('can check if payment is refunded', function (): void {
    $payment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'refunded'])));
...
it('can check if payment requires redirect', function (): void {
    ...
    $successPayment = new Payment(PurchaseData::fromArray(createPurchaseData(['status' => 'success'])));
