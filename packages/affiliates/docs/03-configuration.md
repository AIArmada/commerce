...
Bonus values resolve from the `affiliate-bonuses` settings group first and fall back to this config when settings are unmigrated. Manage them at runtime through the filament-affiliates Bonus Settings page instead of editing config. Bonuses calculate and award via `affiliates:award-bonuses` (or the Performance Bonuses page); performance-bonus rule types are intentionally excluded from per-program commission rules because they run globally.
