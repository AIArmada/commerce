...
class AuthorizePanelRoles
{
    public function handle(Request $request, Closure $next): mixed
    {
        ...
        $enabled = (bool) config('filament-authz.features.panel_role_authorization');
        if ($enabled === false) {
            return $next($request);
        }
        ...
        $roles = (array) (config('filament-authz.panel_roles.'.$panelId) ?? []);
        if ($roles === [] || count($roles) === 0) {
            return $next($request);
        }

        $guardMap = (array) config('filament-authz.panel_guard_map');
