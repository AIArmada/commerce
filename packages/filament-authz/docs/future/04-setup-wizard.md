...
```php
#[AsCommand(name: 'authz:setup')]
class SetupCommand extends Command
{
    public $signature = 'authz:setup
...
php artisan authz:setup

# Minimal setup (no prompts)
php artisan authz:setup --minimal

# Fresh setup (drops existing tables)
php artisan authz:setup --fresh

# Force (no confirmations)
php artisan authz:setup --force

# Quick setup for specific panel
php artisan authz:setup --panel=admin --minimal
```
...
protected function publishConfig(): void
{
    ...
    file_put_contents(config_path('filament-authz.php'), $content);
}
```
