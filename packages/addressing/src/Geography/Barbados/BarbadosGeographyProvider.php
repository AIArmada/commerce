...
class BarbadosGeographyProvider implements CountryAddressAreaMetadataProvider, CountryGeographyProvider, CountryHierarchyProvider, CountryPostalCodeNormalizer
{
    public const string AREA_SOURCE = 'aiarmada_addressing_barbados_v1';

    private const string PROVIDER_KEY = 'aiarmada.addressing.barbados';
    ...
    public function postalCodeLookupKeys(string $code): array
    {
        ...
        // Bundled codes carry the BBprefix; bare input gains it.
        if (preg_match('/^(?:BB?)?(\d{5})$/', $code, $matches) === 1) {
            $canonical = 'BB' . $matches[1];
    ...
    public function areaRoles(AddressCountry $country): array
    {
        ...
        foreach ($this->addressAreaSource()->areas() as $area) {
            ...
            $roles[$area->sourceId] = array_map(
                static fn (string $role): array => ['role' => $role, 'country_code' => 'BB', 'is_primary' => true],
                $areaRoles,
            );
        }
    ...
    public function addressAreaSource(): AddressAreaSource
    {
        return new CsvAddressAreaSource(
            __DIR__ . '/../../../resources/geography/barbados-address-areas.csv',
            self::AREA_SOURCE,
        );
    }
