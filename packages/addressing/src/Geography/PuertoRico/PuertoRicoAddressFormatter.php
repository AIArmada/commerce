...
final class PuertoRicoAddressFormatter implements CountryAddressFormatter
{
    ...
    public function format(AddressData $address): string
    {
        ...
        if ($state !== null && self::sameText($state, 'PR')) {
            $state = null;
        }

        if ($city !== null && $state !== null && ! self::sameText($city, $state)) {
            $lines[] = $city;
            $lines[] = $state . ' PR' . ($postcode !== null ? ' ' . $postcode : '');
        } elseif ($city !== null) {
            $lines[] = $city . ' PR' . ($postcode !== null ? ' ' . $postcode : '');
        } elseif ($state !== null) {
            $lines[] = $state . ' PR' . ($postcode !== null ? ' ' . $postcode : '');
        ...
        if ($address->country !== null && $address->country !== '') {
            ...
            $lines[] = mb_strtoupper($address->countryCode) === 'PR' ? 'Puerto Rico' : $address->countryCode;
        }
