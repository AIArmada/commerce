...
class MoldovaGeographyProvider implements CountryAddressAreaMetadataProvider, CountryAreaTypeLabelProvider, CountryGeographyProvider, CountryHierarchyProvider, CountryPostalCodeNormalizer
{
    ...
    public function postalCodeLookupKeys(string $code): array
    {
        ...
        // Bundled codes carry the MD-prefix; bare input gains it.
        if (preg_match('/^(?:MD-?)?(\d{4})$/', $code, $matches) === 1) {
            $canonical = 'MD-' . $matches[1];
    ...
    {
        // Romanian administrative terms (`city` labels the L2 orașe; the L1 municipii share the type).
        return [
            'district' => 'Raion',
            'commune' => 'Comună',
            'city' => 'Oraș',
