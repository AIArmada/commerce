...
class AzerbaijanGeographyProvider implements CountryAddressAreaMetadataProvider, CountryAreaTypeLabelProvider, CountryGeographyProvider, CountryHierarchyProvider, CountryPostalCodeNormalizer
{
    ...
    public function postalCodeLookupKeys(string $code): array
    {
        ...
        // Bundled codes carry the AZ prefix; bare input gains it.
        if (preg_match('/^(?:AZ\s?)?(\d{4})$/', $code, $matches) === 1) {
            $canonical = 'AZ ' . $matches[1];
