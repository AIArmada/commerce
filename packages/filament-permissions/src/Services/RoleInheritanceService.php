 ...
 * added to the roles table by filament-permissions migrations.
 *
...
class RoleInheritanceService
{
    ...
    public function setParent(Role $role, ?Role $parent): Role
    {
        if ($parent !== null) {
            ...
            // Check depth limit
            $maxDepth = config('filament-permissions.hierarchies.max_role_depth', 5);
    ...
    public function getAncestors(Role $role): Collection
    {
        ...
        $ttl = config('filament-permissions.cache_ttl', 3600);

        if (config('filament-permissions.hierarchies.cache_hierarchy', true)) {
