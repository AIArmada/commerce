...
return [
    'identity_providers' => [
        'ldap' => [
            'enabled' => env('PERMISSIONS_LDAP_ENABLED', false),
            'connection' => 'default',
            'group_mapping' => [
        ...
        'saml' => [
            'enabled' => env('PERMISSIONS_SAML_ENABLED', false),
            'attribute_mapping' => [
...
```bash
# Create snapshot
php artisan permissions:snapshot create "Before major update" --description="Backup before v2.0"

# List snapshots
php artisan permissions:snapshot list

# Compare snapshots
php artisan permissions:snapshot compare snapshot-1 snapshot-2

# Rollback
php artisan permissions:snapshot rollback snapshot-1 --dry-run
php artisan permissions:snapshot rollback snapshot-1 --force
```
...
These features position filament-permissions as a true enterprise solution.
