...
final class PermissionAuditLog extends Model
{
    ...
    {
        return config('filament-permissions.database.tables.audit_logs', 'permission_audit_logs');
    }
...
final class ComplianceReportService
{
    ...
    public function generatePrivilegedAccessReport(): array
    {
        $privilegedRoles = Role::query()
            ->whereIn('name', config('filament-permissions.audit.privileged_roles', ['super_admin', 'admin']))
            ->with('users')
            ->get();
...
```php
#[AsCommand(name: 'permissions:audit-report')]
final class AuditReportCommand extends Command
{
    protected $signature = 'permissions:audit-report 
